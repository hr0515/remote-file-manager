...
class MarkdownPad2AutoCatalog {
  ...
  //openDark   是否开启夜览模式
  constructor(showIndex = false, indexStyle = 3, openDark = false,) {
  ...
  createContent() {
    ...
    leftBlock.innerHTML = '\n<div class="top-container">\n' +
      ...
      '   <i class="iconfont icon-closeC"></i>\n' +
      '   </div>\n';
    //4.设置内容父级元素的内容结构
    ...
    leftBlock.style.padding = '0px';
    leftBlock.classList.add('js-switch-button');
    rightBlock.style.padding = '15px 15px 0px';
    ...
    let msg = '<p class="note-tips">Powered By HR!</p>';
    //5.追加结构元素到页面
    ...
    for (let j = 1; j <= 5; j++) {
      ...
      if (j == 1) {
        for (let k = 0, len = levelLength; k < len; k++) {
          ...
          liElement.innerHTML = '\n<a href="' + href + '#' + levelArr[k].id + '"' + " class=" + levelArr[k].id + '>\n' +
        ...
        for (let n = 0, len = levelLength; n < len; n++) {
          ...
          for (let m = 0, preLen = prevLevelArr.length; m < preLen; m++) {
            ...
            if (currentId == prevId) {
              ...
              liElement.innerHTML = '\n<a href="' + href + '#' + levelArr[n].id + '"' + " class=" + levelArr[n].id + '>\n' +
              ...
              if (currentUlElement !== null) {
                ...
                switch (this.indexStyle) {
                  case 1:
                    classIcon = 'iconfont icon-openA';
                    break;
                  case 2:
                    classIcon = 'iconfont icon-openB';
                    break;
                  case 3:
                    classIcon = 'iconfont icon-openC';
                    break;
                  default:
                    classIcon = 'iconfont icon-openA';
                }
