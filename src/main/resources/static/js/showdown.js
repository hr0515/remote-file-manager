...
showdown.subParser('makehtml.blockQuotes', function (text, options, globals) {
  ...
  // 补丁3：得把 转义的 gt; 重新转回来 不然下面 replace 找不到通配符
  if (text.indexOf('>') === -1) {
    // 找不到接着找 &gt
    if (text.indexOf('&gt;') === -1) {
      // do nothing 找不着不用动说明没有 引用
    }else {
      // 找到了 把 &gt 都换成 >
      text = text.replace(/&gt;/g, '>');
    }
...
showdown.subParser('makehtml.images', function (text, options, globals) {
  ...
  function writeImageTagBaseUrl (wholeMatch, altText, linkId, url, width, height, m5, title) {
    ...
    // 补丁1
    // 构建新的 url
    var ab = 'image?path=' + getAbsolutePath();
    url = ab + url;
...
showdown.subParser('makehtml.lists', function (text, options, globals) {
  ...
  function processListItems (listStr, trimTrailing) {
    ...
    listStr = listStr.replace(rgx, function (wholeMatch, m1, m2, m3, m4, taskbtn, checked) {
      ...
      // 补丁2：每一个li标签里面都应该加一个p标签，她这上面有漏掉的 我给补上
      // 首先要判断item是否含有p标签
      if (item.indexOf('<p>') === -1) {
        item = '<p>' + item + '</p>';
      }
...
showdown.Converter = function (converterOptions) {
  ...
  this.makeHtml = function (text) {
    ...
    // return text;
    // 补丁4 代码返回最后的时候还是有转义字符
    return showdown.helper.unescapeHTMLEntities(text);
  };
